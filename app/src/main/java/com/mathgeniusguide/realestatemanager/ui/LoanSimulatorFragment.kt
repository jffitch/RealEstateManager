    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        calculateButton.setOnClickListener {
            if (timeField.text.isEmpty()) {
                ...
                val rate = if (interestField.text.isEmpty()) 1.0 else Math.exp(interestField.text.toString().toDouble() / 1200)
                ...
                paymentField.text = "%,.2f".format(payment)

                val firstItem = PaymentItem()
                firstItem.balance = String.format(context!!.resources.getString(R.string.dollar_sign), "%,.2f".format(balance.toDouble()))
                ...
                while (balance > 0) {
                    ...
                    paymentItem.principal = String.format(context!!.resources.getString(R.string.dollar_sign), "%,.2f".format(principal))
                    paymentItem.interest = String.format(context!!.resources.getString(R.string.dollar_sign), "%,.2f".format(interest))
                    paymentItem.balance = String.format(context!!.resources.getString(R.string.dollar_sign), "%,.2f".format(balance))
